...
class StudyQuery extends AbstractQuery {
    ...
    static final Expression<?>[] SELECT = {
            QStudy.study.pk,
            QPatientID.patientID.id,
            QPersonName.personName.familyName,
            QPersonName.personName.givenName,
            QStudy.study.studyID,
            QPatient.patient.patientSex,
            QStudy.study.studyDate,
            QSeries.series.modality,
            QSeries.series.bodyPartExamined,
...
//        HibernateQuery<Tuple> q = new HibernateQuery<Void>(session).select(SELECT).from(QStudy.study);
        ...
        HibernateQuery<Tuple> q = new HibernateQuery<Void>(session).select(SELECT)
        												.from(QStudy.study)
        												.leftJoin(QPatient.patient).on(QPatient.patient.pk.longValue().eq(QStudy.study.patient.pk.longValue()))
        												.leftJoin(QPatientID.patientID).on(QPatientID.patientID.pk.longValue().eq(QPatient.patient.patientID.pk.longValue()))
        												.leftJoin(QPersonName.personName).on(QPersonName.personName.pk.longValue().eq(QPatient.patient.patientName.pk.longValue()))
        												.leftJoin(QSeries.series).on(QSeries.series.study.pk.longValue().eq(QStudy.study.patient.pk.longValue()));
        				
        //, QPatient.patient, QPatientID.patientID, QPersonName.personName, QSeries.series
        //q.
