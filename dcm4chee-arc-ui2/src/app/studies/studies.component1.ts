...
@Component({
    ...
    templateUrl: './studies.component1.html'
})
export class StudiesComponent1 implements OnDestroy,OnInit{

    // @ViewChildren(MessagingComponent) msg;

    //@Input() template: TemplateTree;
    tabIndex = 0;
    ...
    // new add
    setCheckItems(){
        $("#findings").val($("#tmpFindings").val());
        $("#inspectionOpinion").val($("#tmpInspectionOpinion").val());
    }
    _ = _;
...
/*        this.ScheduledProcedureStepSequence.ScheduledProcedureStepStartDate.fromObject = null;
    ...
    private init(){
        // this.aet = "MY_AE";
        // this.aetmodel = {};
        // this.aetmodel.dicomAETitle = "MY_AE";
        // this.externalInternalAetModel = {};
        // this.externalInternalAetModel.dicomAETitle = "MY_AE";
...
/*                if(value.vr === "DA" && value.Value && value.Value[0]){
 ...
 /*       $http.get('iod/study.iod.json',{ cache: true}).then(function (res) {
    ...
    modifyPatient(patient, mode , patientkey){
        ...
        $("#patientName").val(patient.attrs['00100010'].Value[0]['Alphabetic']);
        $("#patientSex").val(patient.attrs['00100040'].Value[0]);
        $("#patientNo").val(patient.attrs['00100020'].Value[0]);

        this.queryAllStudiesOfPatient(patient, 0, null, function(obj){});
        $("#patientAge").val(patient.studies[0].attrs['00101010'].Value[0]);
        $("#modality").val(patient.studies[0].attrs['00080061'].Value[0]);
        $("#studyDate").val(patient.studies[0].attrs['00080020'].Value[0]);
        let studieObj = patient.studies[0];
        this.querySeries(studieObj,0, function(obj){
            $("#bodyPartExamined").val(obj[0].attrs['00180015'].Value[0]);
            $("#accessionNumber").val(obj[0].attrs['00200010'].Value[0]);
        });
...
/*        this.cfpLoadingBar.start();
    ...
    querySeries = function(study, offset, callback) {
    ...
    queryAllStudiesOfPatient = function(patient, offset, event, callback) {
